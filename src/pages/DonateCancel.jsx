import { Helmet } from "react-helmet-async";
import { motion } from "framer-motion";
import { XCircle } from "lucide-react";
import { Link } from "react-router-dom";
import { useTranslation } from "react-i18next";
import { Button } from "../components/ui/Button";

const DonateCancel = () => {
  const { t } = useTranslation("donate");
  return (
    <>
      <Helmet>
        <title>{t("cancel.seo_title")}</title>
      </Helmet>
      <main className="min-h-[70vh] flex items-center justify-center bg-slate-50 py-16 px-6">
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="max-w-lg w-full bg-white rounded-3xl p-10 text-center shadow-xl border border-slate-100"
        >
          <div className="mx-auto w-20 h-20 bg-red-500/10 rounded-full flex items-center justify-center mb-6 text-red-500">
            <XCircle className="w-10 h-10" />
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-primary mb-4">
            {t("cancel.title")}
          </h1>
          <p className="text-lg text-slate-600 mb-8 leading-relaxed">
            {t("cancel.message")}
          </p>

          <Link to="/donate">
            <Button className="w-full h-14 text-lg">{t("cancel.buttons.try_again")}</Button>
          </Link>
        </motion.div>
      </main>
    </>
  );
};

export default DonateCancel;
